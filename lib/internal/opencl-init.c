...
unsigned acc_get_platform_desc(cl_platform_id platform) {
  ...
  for (r = 0; r < ACC_NUM_PLATFORMS; r++)
    if (strcmp(name, platforms_desc[r].ocl_name) == 0)
      break;
  if (r == ACC_NUM_PLATFORMS) {
    printf("[warn]    Unrecognized OpenCL Platform: \"%s\"\n", name);
  }
...
unsigned acc_get_device_desc(cl_device_id * device, unsigned r) {
  ...
  for (t = 0; t < platforms_desc[r].num_devices; t++) {
    ...
    printf("compare %s\n     to %s\n return %s\n",
            name,
            platforms_desc[r].devices_desc[t].ocl_name,
            strstr(name, platforms_desc[r].devices_desc[t].ocl_name)
          );
#endif
  ...
  if (t == platforms_desc[r].num_devices) {
    printf("[warn]    Unrecognized OpenCL Device : %s\n", name);
  }
...
void acc_register_platform_desc(unsigned r, unsigned first, size_t num) {
  acc_device_t device = platforms_desc[r].device;

  assert(device != acc_device_last);
  ...
  for (i = 0; i < acc_runtime.opencl_data->num_platforms; i++) {
    ...
    if (r != ACC_NUM_PLATFORMS) {
      ...
      for (j = 1; j < acc_runtime.opencl_data->num_devices[i]; j++) {
        ...
        if (t != prev_t) {
          if (prev_t != platforms_desc[r].num_devices)
      ...
      assert( (base_t + cnt_t == idx + acc_runtime.opencl_data->num_devices[i]) );
    }
